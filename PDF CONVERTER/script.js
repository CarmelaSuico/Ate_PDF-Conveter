...
function createPDF() {
    ...
    // Year to date CIs Deductible Pay and Deduction
    const deductiblePayYTD = parseFloat(document.getElementById("deductiblePay").value) || 0;
    const deductionYTD = parseFloat(document.getElementById("deductionyeartodate").value) || 0;

    const totalpayment = monthlyPay + materials + vat;
    ...
    rowPayments.forEach(([label, value]) => {
        ...
        setTextStyle(doc, "value");
        doc.text(`£${formattedValue}`, t2X + boxWidth - padding, t2Y_Internal, { align: "right" });
    ...
    doc.text(`£${formatCurrency(totalpayment)}`, t2X + boxWidth - padding, t2BottomY, { align: "right" });
    ...
    //values
    setTextStyle(doc, "value");
    doc.text(`£${formatCurrency(deduction)}`, t3X + boxWidth - padding, t3Y_Internal, { align: "right" });
    ...
    // value
    setTextStyle(doc, "totalValue");
    doc.text(`£${formatCurrency(totaldeduction)}`, t3X + boxWidth - padding, t3BottomY, { align: "right" });
...
// --------------------------------------------------------RESULT----------------------------------------------------------------
    ...
    setTextStyle(doc, "value");
    doc.text(`£${formatCurrency(monthlyPay)}`, t1X + box2Width - padding, row2TableMargin, { align: "right" });
    ...
    // Top Items
    const row2Payments = [
        ["CIS deductible pay", deductiblePayYTD.toFixed(2)],
        ["CIS deduction", deductionYTD.toFixed(2)]
    ];
    ...
    row2Payments.forEach(([label, value]) => {
        ...
        const formattedValue = formatCurrency(parseFloat(value));
        setTextStyle(doc, "value");
        doc.text(`£${formattedValue}`, t2X + boxWidth - padding, resultt2Y_Internal, { align: "right" });
    ...
    //table6
    const TOTALOFALL = totalpayment - totaldeduction;
    doc.setFillColor(255, 255, 255);
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(30);
    doc.text(`£${formatCurrency(TOTALOFALL)}`, centerX, centerY, { align: "center" });
    doc.setFont("times", "normal");
    doc.setFontSize(12);
...
function downloadPDF() {
    const name = document.getElementById("name").value || "N/A";
    const date = document.getElementById("date").value;
    const numericDate = formatDate(date, 'pdf_name');
